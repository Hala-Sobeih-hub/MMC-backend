...
router.post('/request-deletion', authMiddleware, async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: 'Your deletion request has been submitted and is pending approval.',
            user,
        });
    } catch (err) {
        console.error('Request Deletion Error:', err.message);
        res.status(500).json({ message: 'An error occurred while submitting your deletion request. Please try again later.' });
    }
});
...
router.patch('/deletion-requests/:userId', authMiddleware, async (req, res) => {
    try {
        ...
        if (!user.deletionRequest) {
            return res.status(400).json({ message: 'This user has not requested account deletion.' });
        }
        ...
        if (action === 'approve') {
            ...
            res.status(200).json({ message: 'Deletion request approved and user account deleted.' });
        ...
        res.status(500).json({ message: 'An error occurred while processing the deletion request. Please try again later.' });
    }
});
